...
def rbcua():
    # print('success')
    # getting data
    data = requests.get("https://www.rbc.ua/ukr/news", headers={
    ...
    for title in soup.find('div', {'class': 'content-section'}).find_all('div', {'class': 'news-feed-item'}):
        try:
            # print('found')

            title_text = title.find('a').get_text()[7:]
            title_text = re.sub('\n', '', title_text)
            # print(title_text)

            link = title.find('a').get('href')
            if not link.startswith('https://'):
                link = "https://www.rbc.ua/ukr" + link
            # print(link)
    ...
    if len(articles) < 30:
        try:
            ...
            requests.get('https://api.telegram.org/bot{}/sendMessage?chat_id=373407132&text={}'.format(TOKEN2, 'Проблема з парсингом RBC ua'))
        except ImportError:
