...
def zero32lviv():
    ...
    # print('success')
    # getting data
    data = requests.get("https://www.032.ua/news", headers={
    ...
    for title in soup.find('div', {'class': 'col-lg-9'}).find_all('div', {'class': 'c-news-card'}):
        # print('found')
        try:
            title_text = title.find('div', {'class': 'c-news-card__head'}).find('a').get_text()
            # print(title_text)
    ...
    if len(articles) < 7:
        try:
            ...
            requests.get('https://api.telegram.org/bot{}/sendMessage?chat_id=373407132&text={}'.format(TOKEN2, 'Проблема з парсингом 032lviv'))
        except ImportError:
