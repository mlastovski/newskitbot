...
def hromadske():
    ...
    for title in soup.find('div', {'class': 'wrapper'}).find_all('div', {'class': 'item-wrapper'}):
        try:
            ...
            if not link.startswith('https://'):
                link = 'https://hromadske.ua/news' + link
            print(link)
    ...
    if len(articles) < 5:
        try:
            ...
            requests.get('https://api.telegram.org/bot{}/sendMessage?chat_id=373407132&text={}'.format(TOKEN, 'Проблема з парсингом Hromadske'))
        except ImportError:
